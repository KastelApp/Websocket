...
export class Cache {
   ...
   * @param {number|string} [port] The port the host is listening on
   * @param {string} [user] The user (The default user is 'default')
   * @param {string} [password] The Password (If one is required if not put null)
   ...
   */
  async isCached(variable: string, key?: string): Promise<boolean> {
  ...
  async set(variable: string, key: string, item?: string): Promise<'OK'> {
  ...
  async get(variable: string, key?: string): Promise<string | null> {
    return new Promise((resolve, reject) => {
      this.redis
        .get(`${key ? `${variable}:${key}` : variable}`)
        .then((x) => resolve(x))
  ...
  async keys(variable: string): Promise<string[]> {
    return new Promise((resolve, reject) => {
      this.redis
  ...
  async reset(variable: string, key?: string, item?: string) {
    return new Promise((resolve, reject) => {
      const multi = {
  ...
  async delete(key: string): Promise<number | void> {
    return new Promise((resolve, reject) => {
      this.redis
        .del(key)
        .catch((error) => reject(error))
   ...
   */
  async clear(ignore?: string[] | string): Promise<string[]> {
    return new Promise((resolve, reject) => {
      this.redis
        .keys('*')
        .then((RedisKeys) => {
          ...
          if (ignore) {
            ...
            for (const e of RedisKeys) {
              if (Array.isArray(ignore)) {
                ...
                if (ignore.includes(keys as string)) continue;
              } else if (typeof ignore === 'string' && e.split(':')[0] == ignore) continue;
              NotIgnored.push(e);
            }
